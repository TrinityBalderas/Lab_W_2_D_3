...
callBackfn(result, num1, num2)
}
function outputResult(num1){
console.log(`The answer is ${num1}`);
}
function fullSentence(result, num1, num2){
console.log(`${num1} times ${num2} equals ${result}`);
}
// UNCOMMENT THE FUNCTION CALL BELOW
// RUN THIS FILE WITH NODE
// CHECK YOUR ANSWER

//multiply(4, 3, fullSentence)
//multiply(12, 8, outputResult)
...
function first(names, callBack2){
  callBack2(names[0])
}
function callBack2(name){
  console.log('The first name in names is ' + name)
}
// UNCOMMENT THE FUNCTION CALL BELOW
// RUN THIS FILE WITH NODE
// CHECK YOUR ANSWER

 //first(names, callBack2)
...
callBack3(boolResult, name)
}
function callBack3(result, name){
if(result === true){
...
function uniq(arr, callBack4){
for(i = 0; i < arr.length; i++){
  for(x = 0; i < arr.length; i++){
    if(arr[i] === arr[x]){
      arr.splice(x, 1)
    }
  }
...
function callBack4(uniqArr){
console.log(`The new names array with all the duplicate items removed is ${uniqArr}.`);
}
//uniq(names, callBack4)
...
function each(arr, callBack5){
for(i = 0; i < arr.length; i++){
callBack5(arr[i], i)
}
}

//uniq(names, callBack5)
/*
...
function callBack5(name, num){
  console.log(`${num} is ${name}`);
  }
...
//getUserById(users, '16t', callback6)
